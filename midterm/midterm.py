...
def create_tire(name, width, radius, tx, ty, tz):
    ...
    # Return the transform node name.
    tire = cmds.polyCylinder(h=width, r=radius, ax=(0,0,1),sx=20,sy=1, sc=True, name=name)
...
""" the answer is here from reading "def arPolyNoise() with two arugment object to apply the effect and the noise strength', but how do I apply it to the car I'm not sure, I tried to go indivually, I tried as a group.
 I'm trying to apply the noises to the transforamtion nod for the objects using node name [0], I tried to go from Hypergraphic:Connections menu, tried to go inside the other function that create the car's parts. I'm not what I ned to do if possible my focuse is on the wrong places.""" 
#also tried to switch functions order (polynoise function & creation of car functions) 
